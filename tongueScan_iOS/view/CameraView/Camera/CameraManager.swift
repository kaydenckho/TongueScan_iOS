...
class CameraManager: ObservableObject {
  ...
  private func set(error: CameraError?){
    ...
    func isUsingFrontCamera() -> Bool {
        return getCurrentCameraPosition() == .front
    }

    func isUsingBackCamera() -> Bool {
        return getCurrentCameraPosition() == .back
    }
    ...
    func switchCamera() {
        sessionQueue.async {
            ...
            do {
                ...
                // Add new input if possible, otherwise restore old
                ...
                if connection.isVideoMirroringSupported {
                    let isFront = self.device?.position == .front
                    connection.isVideoMirrored = isFront
                }
                
                self.session.commitConfiguration()
            } catch {
                // If creation fails, leave session as-is
