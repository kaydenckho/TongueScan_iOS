...
class CameraViewVM: ObservableObject {
    ...
    func takePicture() {
        ...
        frameManager.captureCallback = { cgImage in
            Task{
                let filename = "TongueScan_" + dateFormatter.string(from: date)
        ...
        frameManager.captureCallbackFlash = { cgImage in
            Task{
                let filename = "TongueScan_flash_" + dateFormatter.string(from: date)
                self.cropAndAppendImage(filename: filename, cgImage: cgImage)
    ...
    func cropAndAppendImage(filename: String, cgImage: CGImage) {
        ...
        // Determine orientation based on camera
        ...
        let originalImgData = rotateImage(cgImage: finalOriginalImg, orientation: orientation).jpegData(compressionQuality: 0.8)
    ...
    // Helper function for horizontal mirroring
    func mirrorImage(cgImage: CGImage) -> CGImage? {
